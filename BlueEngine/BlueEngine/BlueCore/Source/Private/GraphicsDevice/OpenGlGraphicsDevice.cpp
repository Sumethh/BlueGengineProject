...
#define ASSERT_NO_GRAPHICS_ERROR()if(GLenum err = glGetError()){Log::LogError(std::to_string((int)err)); Log::Flush(); BlueAssert(false);}
	...
	inline GLenum GetGlEnumFromDrawMode(EDrawMode aMode)
	{
		switch (aMode)
		{
			case BlueCore::EDrawMode::Triangles:
				return GL_TRIANGLES;
				break;

			case BlueCore::EDrawMode::Lines:
				return GL_LINES;
				break;
	...
	int GetGlEnumFromResourceType(EGraphicsResourceType aType)
	{
		switch (aType)
		{
			case BlueCore::EGraphicsResourceType::VertexArrayBuffer  :
				return GL_VERTEX_ARRAY;
				break;

			case BlueCore::EGraphicsResourceType::VertexBuffer:
				return GL_ARRAY_BUFFER;
				break;

			case BlueCore::EGraphicsResourceType::ElementBuffer:
				return GL_ELEMENT_ARRAY_BUFFER;
				break;

			case BlueCore::EGraphicsResourceType::Texture2D:
				return GL_TEXTURE_2D;
				break;

			case BlueCore::EGraphicsResourceType::Shader:
				BlueAssert(false);
				return 0;
				break;

			case BlueCore::EGraphicsResourceType::Count:
				BlueAssert(false);
				return 0;
				break;
