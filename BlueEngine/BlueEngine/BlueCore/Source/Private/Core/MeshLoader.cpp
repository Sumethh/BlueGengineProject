...
namespace BlueCore
{
	void LoadMesh(const aiScene& aScene, Mesh* aMesh, uint32 aMeshIndex)
	{
		...
		BlueCore::Vertex* meshVertex = new BlueCore::Vertex[vertCount];

		for (size_t i = 0; i < vertCount; i++)
		{
			BlueCore::Vertex newVert = {};
	...
	BlueCore::Mesh* MeshLoader::LoadMeshFromFile(std::string aFilePath, std::string aMeshName, Mesh* aMesh)
	{
		Assimp::Importer importer;
